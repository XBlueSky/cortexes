...
| `_repo_index.json` | `/synosrc/cortex/` | Yes |
...
cortex-vec search "FSDN" --tags fsdn
...
- `--tags <tag>` — filter by tags (substring match in comma-separated field)
...
Export `_repo_index.json` from ChromaDB metadata.

```bash
cortex-vec export-repo-index
```

1. Query all documents with `repo` metadata
2. Group by repo → split into projects/notes by type
3. Write to `<vault>/_repo_index.json`
4. Report: `Exported: N repos to _repo_index.json`

This replaces `scripts/build-repo-index.sh`.
...
Last rebuild: 2026-04-14
```

### CLI output conventions

- Default output: JSON (machine-readable, for skills to parse)
- `--plain` flag: human-readable format
- Exit 0: success
- Exit 1: error (message to stderr)
- Quiet on success for upsert/delete (unless `--verbose`)

## Integration points

### session-start hook

No change to the hook itself. It continues to read `_repo_index.json` with jq. The `_repo_index.json` is now generated by `cortex-vec export-repo-index` instead of `build-repo-index.sh`.

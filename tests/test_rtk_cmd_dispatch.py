...
from rtk_cmd.mcp_tools import filter_mcp_tool, filter_robinhood  # noqa: E402
...
class FindMcpFilter(unittest.TestCase):
    ...
    def test_build_mcp_build_toolkit_drift(self):
        self.assertIs(find_mcp_filter("mcp__plugin_synology-build-toolkit_syno-build-mcp__docker_execute"),
                      filter_mcp_tool)

    def test_build_mcp_dev_suite(self):
        self.assertIs(find_mcp_filter("mcp__plugin_synology-dev-suite_syno-build-mcp__build_project"),
                      filter_mcp_tool)

    def test_naxos_dev_suite(self):
        self.assertIs(find_mcp_filter("mcp__plugin_synology-dev-suite_syno-naxos__exec_command"),
                      filter_naxos)

    def test_naxos_diagnostics_drift(self):
        self.assertIs(find_mcp_filter("mcp__plugin_synology-diagnostics_syno-naxos__exec_command"),
                      filter_naxos)

    def test_robinhood(self):
        self.assertIs(find_mcp_filter("mcp__plugin_syno-robinhood_robinhood__codesearch"),
                      filter_robinhood)

    def test_gitlab_still_routed(self):
        self.assertIs(find_mcp_filter("mcp__plugin_synology-workflows_gitlab__get_merge_request"),
                      filter_mcp_tool)
